...
from pyparsing import (
    ...
    Optional,
    Or,
    Regex,  # noqa: F401  # used dynamically inside exec() when building credential grammars
    Word,
    WordEnd,
    WordStart,
    alphanums,
...
def pyparsing_parse_result_get_token_dict(parse_result: pyparsing.ParseResults):
    """."""
    token_dict = parse_result._tokdict
...
# TODO: not sure if there are other ways to declare a variable in javascript which could easily be found with a
# grammar - I could also find arguments to a given function
...
yara_rule_scopes = OneOrMore(Or(yara_rule_scopes_list))  # type: ignore[arg-type]  # pyparsing accepts str literals
...
# for the rule body, find any printable character and make sure the last character is a '}' representing the end of
# the yara rule
rule_body = OneOrMore(
